...
def mostrar_logs(tipo):
    try:
        ...
        for linea in lineas[-30:]:  # Mostrar ultimas 30 l赤neas,para no tener demasiadas lineas
            print(linea.strip())
...
def cp(args):
    if (len(args) != 2):  # Si la cantidad de argumentos recibidos es diferente a 2, se abandona la funcion
        ...
        try:
            ...
            print(f"Ocurri車 un error durante la copia: {e}")
...
def parseador(linea_comando):
    ...
    token_actual = ""  # Acumilador del token de construcci車n
    ...
    for caracter in linea_comando:  # Leer caracter por caracter
        if proximo_escape:
            ...
            if token_actual:
                tokens.append(token_actual)  # Se termin車 el token, se agrega al arrays de tokens
                token_actual = ""  # Comenzamos un nuevo token
        else:
            # Caracter normal - agregar al token actual
            token_actual += caracter
    # Agregar el 迆ltimo token si queda alguno, por las dudas.
    if token_actual:
...
def ejecutar_externo(comando,
    ...
    if pid == 0:  # Si se ejecuta actualmente el hijo
        try:
            ...
            sys.exit(127)  # C車digo est芍ndar linux para "Command not found"
        except Exception as e:
            print(f"Error al ejecutar: {e}")
            sys.exit(1)  # Salir con error gen谷rico
...
def main():
    ...
    while True:
        try:
            ...
            if not partes:
                continue  # Ignorar si el parseador no devolvi車 tokens (ej: solo un '\' o espacios)
            ...
            if comando_principal == "exit":
                ...
                # Implementaci車n manual del comando pwd
                print(os.getcwd())
